...
1. ~~**`main.py` is still 10.5k lines.**~~ ✅ **Done** — split to **2,899 lines** across 15 new modules (`store`, `runtime`, `tasks`, `scaffolding`, `git_ops`, `attention`, `ideas`, `dev_servers`, `quota`, `agent_io`, `registry`, `transcripts`, `runner_policy`, `publish`, `agent_runner`, `workspace`, `orchestration_runs`). The package is acyclic and `test_module_layering.py` holds a 3,000-line ratchet on `main.py`. What remains there is the composition root: app construction, middleware, lifespan, the watchdog, the snapshot builder, and the 43 route handlers.
